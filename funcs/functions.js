...
const initializingApi = async (path, data) => {
    ...
    if (data.module === "commonjs") {
        ...
        await Runner(`cd ${path} && git clone https://github.com/hassaammgl/commonjs-server-create-my-mern-app.git server && cd server && rm -r .git && npm i cors express cookie-parser mongoose dotenv && npm update cors express mongoose dotenv -S`)
        logs.success("initialized commonjs api...")
    }
    else {
        logs.info("initializing ESM api...")
        await Runner(`cd ${path} && git clone https://github.com/hassaammgl/ESM-server-create-my-mern-app.git server && cd server && rm -r .git && npm i cors express cookie-parser mongoose dotenv && npm update cors express mongoose dotenv -S`)
...
const initializingClient = async (path) => {
    ...
    await Runner(`cd ${path} && git clone https://github.com/hassaammgl/client-side-create-my-mern-app.git client && cd client && rm -r .git && npm update @types/react @types/react-dom @vitejs/plugin-react-swc eslint eslint-plugin-react eslint-plugin-react-hooks eslint-plugin-react-refresh vite -S && npm i axios react-router-dom react-hot-toast`);
